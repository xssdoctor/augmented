...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='AI Swiss army knife by Daniel Miessler')
    ...
    parser.add_argument(
        '--stream', '-s', help='use this option is you are piping output to another app. The output will not be streamed', action='store_true')
    parser.add_argument('--pattern', '-p', help='module to use')
    parser.add_argument(
        '--server', '-S', help='Server mode!!!', action='store_true')
    parser.add_argument('--domain', '-d', help='domain to use for server mode')
    parser.add_argument('--port', '-P', help='port to use for server mode')
    parser.add_argument('--apikey', '-a', help='add a chatgpt api key')
    ...
    if args.apikey:
        ...
        print(f'api key set to {args.apikey}')
        # quit
        sys.exit()
    if args.server:
        server = Server()
